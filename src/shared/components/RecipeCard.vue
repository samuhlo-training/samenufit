<template>
  ...
  <div
    v-if="!recipe"
    class="bg-[#fff3e3] rounded-lg shadow-sm overflow-hidden min-w-[22rem] border-2 border-dashed border-[var(--accent-color)]/30 hover:border-[var(--accent-color)]/80 transition-colors duration-300 cursor-pointer"
  >
    <button
      @click="handleEmptyClick"
      class="w-full p-6 flex flex-col items-center justify-center hover:bg-white/10 transition-colors duration-200 cursor-pointer text-center group"
    >
      <div class="mb-2">
        <svg
          class="w-8 h-8 text-[var(--accent-color)] mx-auto transition-transform duration-200 ease-in-out group-hover:scale-130"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
  ...
  <div v-else class="bg-[#fff3e3] rounded-lg shadow-sm overflow-hidden min-w-[22rem]">
    ...
    >
      <div class="text-left">
        <div class="text-m text-text-main-color opacity-80">{{ recipe.type }}</div>
    ...
    >
      <div v-show="isExpanded" class="overflow-hidden">
        <div class="px-4 pb-4 border-t border-white/20">
          <!-- Ingredientes -->
          <div class="mb-4">
